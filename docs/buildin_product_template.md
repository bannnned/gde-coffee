...
#### reputation_v1 (0-1000)

```text
rep_raw = 100 + Σ(points(event_k) * decay(age_k))

decay(age_k):
  1.0, если событие <= 365 дней
  0.5, если событие старше 365 дней

События:
+2 * voter_weight      за каждый полученный "полезно"
+4                     за отзыв с quality >= 70
+3                     за verified visit (medium/high)
+2                     за одобренное фото меню
+3                     за одобренное обновление данных кофейни
-25                    за подтвержденную жалобу на отзыв
-35                    за удаление отзыва модератором
-80                    за высокий fraud-сигнал
-15                    за ложную жалобу

Ограничения:
positive_cap_per_day = +25
negative_cap_per_day = -120

reputation_v1 = clamp(rep_raw, 0, 1000)
```
...
| P0 | Построить воронку: карточка -> отзыв -> маршрут -> check-in -> отзыв | [ ] | Сквозная аналитика доступна | _указать_ |
